    ...
    <main class="flex-grow max-w-6xl mx-auto px-4 py-12 w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <section class="lg:col-span-5">
            ...
            <div id="resultado-vazio" class="glass-panel rounded-3xl p-8 text-center border-dashed border-2 border-panelBorder flex flex-col items-center justify-center py-20">
                <svg id="icone-vazio" xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-slate-600 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <svg id="spinner-vazio" class="animate-spin h-16 w-16 text-emerald-400 mb-4 hidden" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
                <h3 id="titulo-vazio" class="text-lg font-medium text-slate-400">@lang('site.aguardando')</h3>
                <p id="desc-vazio" class="text-sm text-slate-500 mt-2 max-w-xs">@lang('site.aguardando_desc')</p>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const STR = {
                ...
                analisandoDesc: @json(__('site.analisando_desc')),
            ...
            const setLoading = (loading) => {
                ...
                // Limpa o resultado anterior e mostra estado de carregamento,
                // para o cliente perceber que os dados mudaram a cada tentativa.
                if (loading) {
                    cardResultado.classList.add('hidden');
                    cardVazio.classList.remove('hidden');
                    document.getElementById('icone-vazio').classList.add('hidden');
                    document.getElementById('spinner-vazio').classList.remove('hidden');
                    document.getElementById('titulo-vazio').textContent = STR.analisando;
                    document.getElementById('desc-vazio').textContent = STR.analisandoDesc;
                } else {
                    document.getElementById('icone-vazio').classList.remove('hidden');
                    document.getElementById('spinner-vazio').classList.add('hidden');
                }
            };
